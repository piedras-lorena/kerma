...
df_submuestras <- read_csv('Proyectos/Otros/kerma/data/interim/submuestras.csv',
                          col_types = cols(.default = "c"))

id_submuestras <- df_submuestras$submuestra %>% unique()  


crear_tabla_reporte <- function(id_submuestra){
  ...
  # Importamos tablas -------------------------------------------------------
  submuestras <- read_csv('Proyectos/Otros/kerma/data/interim/submuestras.csv',
                          col_types = list(
                            id = col_integer(),
                            submuestra = col_character()
                          )) %>% filter(submuestra == id_submuestra)
  ...
  tabla_reporte <- read_csv('Proyectos/Otros/kerma/data/interim/tabla_reporte_prueba.csv',
    ...
    filter(userId %in% submuestras$id)
  ...
  diccionario_diferentes <- read_csv('Proyectos/Otros/kerma/data/interim/tablas_resumen_diferentes - Hoja 1.csv',
    ...
    mutate(seccion_solo = ifelse(is.na(subseccion) & is.na(renglon),seccion,"0"),
           seccion_entera = ifelse(is.na(subseccion) & !is.na(renglon), paste(seccion,'0',renglon,sep='_'),
                                   ifelse(!is.na(subseccion) & !is.na(renglon),
                                          paste(seccion,subseccion,renglon,sep='_'),NA)),
  ...
  operaciones <- read_csv('Proyectos/Otros/kerma/data/interim/operaciones_preguntas - col_1.csv',
    ...
    mutate(columna_1 = ifelse(is.na(categoria_pregunta_1),pregunta_1,paste(pregunta_1,categoria_pregunta_1,sep='_')),
           ...
           nombre_columna_nueva = ifelse(is.na(subseccion),paste(seccion,'0',renglon,sep='_') , 
  ...
  write_csv(mapeo_nombres , sprintf('Proyectos/Otros/kerma/data/interim/submuestras/mappeo_nombres_resumen_reporte_sub%s.csv',id_submuestra))
  write_csv(tabla_final , sprintf('Proyectos/Otros/kerma/data/interim/submuestras/tabla_resumen_final_sub%s.csv',id_submuestra))
}

for(id_submuestra in id_submuestras){
  crear_tabla_reporte(id_submuestra)
}